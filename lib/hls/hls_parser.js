...
shaka.hls.HlsParser = class {
  ...
  createSegmentReference_(
    ...
    const timestampTag = shaka.hls.Utils.getFirstTagWithName(tags, 'EXT-X-PROGRAM-DATE-TIME');
    var timestamp = timestampTag != null ? new Date(Date.parse(timestampTag.value)) : null;
    if (timestamp == null && previousReference != null && previousReference.timestamp != null) {
      const durationMillis = (endTime - startTime) * 1000;
      timestamp = new Date(previousReference.timestamp.valueOf() + durationMillis)
    }
