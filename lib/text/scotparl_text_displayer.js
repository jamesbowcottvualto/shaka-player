...
/**
  SCOTPARL: This is a fork of SimpleTextDisplayer which:
  - Forces center alignment
  - Forces cues to be at 75% from the top of the video
 */
...
goog.provide('shaka.text.ScotparlTextDisplayer');
 ...
 * @export
 */
shaka.text.ScotparlTextDisplayer = class {
  ...
  remove(start, end) {
    ...
    shaka.text.ScotparlTextDisplayer.removeWhere_(this.textTrack_, removeInRange);
  ...
  append(cues) {
    ...
    for (const inCue of flattenedCues) {
      ...
      // SCOTPARL: Force position to 75% from top
      inCue.line = 75;
      inCue.lineInterpretation = 1;
      ...
      if (!containsCue) {
        const cue =
            shaka.text.ScotparlTextDisplayer.convertToTextTrackCue_(inCue);
        if (cue) {
  ...
  destroy() {
    if (this.textTrack_) {
      ...
      shaka.text.ScotparlTextDisplayer.removeWhere_(this.textTrack_, removeIt);
